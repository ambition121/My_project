...
DDR_on));
  ...
  videotestsrc = gst_element_factory_make ("v4l2src", "videotestsrc");
  ...
  gst_bin_add_many (GST_BIN (g_pipeline), videotestsrc, filter, videoenc,
      videopay, NULL);

  if (!gst_element_link_many (videotestsrc, filter, videoenc,
          videopay, NULL)) {
    g_error ("Failed to link videotestsrc, video encoder and video payloader");
  }
