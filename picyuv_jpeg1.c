...
{  
    ...
    if(ioctl(fd, VIDIOC_S_FMT, &fmt) == -1) //VIDIOC_S_FMT设置当前驱动的捕获视频的格式 
    {  
        printf("Unable to set format\n");  
        return FALSE;  
    }       
    if(ioctl(fd, VIDIOC_G_FMT, &fmt) == -1) //VIDIOC_G_FMT读取得到当前驱动的捕获格式 
    {  
...
int v4l2_grab(void) //驱动向内核申请缓冲，并且映射到用户空间 
{  
...
//YUV---YCrCb
    ...
    r1 = y1 + 1.042*(u-128);  
    g1 = y1 - 0.34414*(v-128) - 0.71414*(u-128);  
    b1 = y1 + 1.772*(v-128);  
  
    r2 = y2 + 1.042*(u-128);  
    g2 = y2 - 0.34414*(v-128) - 0.71414*(u-128);  
    b2 = y2 + 1.772*(v-128);
...
{  
    ...
    FILE *fptr_jpg = fopen (JPEG,"wb");//注意这里为什么用fopen而不用open,为什么要以二进制方式打开  
    if(fptr_jpg==NULL)  
    {  
