...
void * send_gsm(void * arg)
{
 ...
 //       int sockfd = (int)arg;//at 10.12
		...
		switch(stat){
			...
			case SENDNUM:  //AT+CMGS="10010",send to 10010 and the server want to send the message to the number
				...
				else if(return_flag == 1)
				{
					...
					bzero(buffer9,20); //init the buffer9 to 0
				...
				//	memcpy(buffer9,return_detail,5);
					...
					nwrite = write(portfd,buffer8,strlen(buffer8));
						...
						printf("buffer8 is:%s\n",buffer8);
					sleep(2);
					nwrite = write(portfd,buffer9,strlen(buffer9));
						printf("!!write the number len is:%d\n",nwrite);
						printf("buffer9 is:%s\n",buffer9);
					sleep(60);
					stat = CHECK;
			...
			if(check_flow == 1)  //we only send the lost flow
			{
				check_flow = 0;
				char *string = NULL;
				char buffer[10];
				...
				{
					char nomessage[] = {0xF2,0x14,0x05,0x01,0x0C}; //stand we couldn't match the lost flow
    		...
    		//	bzero(out,50);
				...
				char strout[] = ",";
    			char strin[] = "\"";
    			between_character(thestring, strout, strin , out);
			...
			**/
			...
			//	memcpy(&atcommand_to_servermesg[14],buff,nread/*strlen(buff)*/); //send the whole message to server
					...
					int j;
					char checksum1 = 0x00;
					for( j=0;j<nread+15;j++ )
						checksum1 += atcommand_to_servermesg[j];
						atcommand_to_servermesg[nread+15] = checksum1;
						atcommand_to_servermesg[nread+16] = '\0';
	...
	printf("3.!!!!!!!!!#####is:%d\n",atcommand_to_servermesg[3]);
						...
						for( j=0;j<254;j++ )
							checksum1 += atcommand_to_servermesg[j];
							atcommand_to_servermesg[254] = checksum1;
							atcommand_to_servermesg[255] = '\0';
	printf("4.!!!!!!!!!!!!!is:%d\n",atcommand_to_servermesg[3]);
	printf("5.the length is:%d\n",strlen(atcommand_to_servermesg));
			...
			//	memcpy(&atcommand_to_servermesg[14],buff,nread/*strlen(buff)*/); //send the whole message to server
					...
					for( j=0; j<b+15; j++ )
						checksum1 += atcommand_to_servermesg[j];
						atcommand_to_servermesg[b+15] = checksum1;
						atcommand_to_servermesg[b+16] = '\0';
					sleep(5);
